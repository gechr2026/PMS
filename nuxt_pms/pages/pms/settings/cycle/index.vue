    ...
    <div>
        ...
        <div class="mb-5 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <h1 class="text-lg font-bold text-gray-800">รอบการประเมินผล</h1>
            </div>
            <NuxtLink
                to="/pms/settings/cycle/add"
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-200 bg-gray-50">
                            ...
                            <th class="w-24 px-4 py-3 text-center font-semibold text-gray-700">ปี</th>
                            <th class="px-4 py-3 text-center font-semibold text-gray-700">รอบการประเมิน</th>
                            <th class="px-4 py-3 text-center font-semibold text-gray-700">วันที่เริ่มต้น</th>
                            <th class="px-4 py-3 text-center font-semibold text-gray-700">วันที่สิ้นสุด</th>
                    ...
                    <tbody>
                        ...
                        <tr v-else-if="cycleList.length === 0">
                        ...
                        >
                            ...
                            <td class="px-4 py-3 text-center text-gray-800">{{ item.year }}</td>
                            <td class="px-4 py-3 text-center text-gray-800">{{ item.cycle_label }}</td>
                            <td class="px-4 py-3 text-center text-gray-600">{{ item.start_date }}</td>
                            <td class="px-4 py-3 text-center text-gray-600">{{ item.end_date }}</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    <!-- Edit -->
                                    <NuxtLink
                                        :to="`/pms/settings/cycle/add?id=${item.id}`"
            ...
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
                <div class="mb-4 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">คุณต้องการลบรอบ {{ deleteTarget?.cycle }} ปี {{ deleteTarget?.year }} ใช่หรือไม่?</p>
                    </div>
                </div>
            ...
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
                <div class="mb-4 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">คุณต้องการรีเซ็ตรอบ {{ resetTarget?.cycle }} ปี {{ resetTarget?.year }} ใช่หรือไม่?</p>
                    </div>
                </div>
                <div class="flex justify-end gap-2">
                    ...
                    <button class="rounded-lg px-4 py-2 text-sm font-semibold text-white transition hover:opacity-90" style="background:#06b6d4;" @click="executeReset">รีเซ็ต</button>
                </div>
            </div>
        </div>
    </div>
...
useHead({ title: 'รอบการประเมินผล | ระบบประเมินผลการปฏิบัติงาน' });
...
const fetchYears = async () => {
    try {
        ...
        // sort year value DESC
        yearOptions.value = res.data.slice().sort((a, b) => b.year - a.year);
    } catch (e) {
        console.warn('[cycle] failed to load year options', e);
    }
};
...
const fetchList = async () => {
    ...
    try {
        const res = await cyclesApi.list({
            ...
            cycle: searchCycle.value.trim() || undefined,
...
const handleClear  = () => {
    searchYear.value = ''; searchCycle.value = ''; searchStatus.value = '';
    fetchList();
};

const toggleStatus = async (item: PmsCycle) => {
    togglingId.value = item.id;
    errorMessage.value = '';
    const newValue = !item.is_active;
    try {
        const res = await cyclesApi.update(item.id, { is_active: newValue });
        // replace item in list
        const idx = cycleList.value.findIndex(c => c.id === item.id);
        if (idx >= 0) cycleList.value[idx] = res.data;
    } catch (e) {
...
const deleteTarget    = ref<PmsCycle | null>(null);
const confirmDelete   = (item: PmsCycle) => { deleteTarget.value = item; showDeleteModal.value = true; };
const executeDelete   = async () => {
    if (!deleteTarget.value) return;
    deleting.value = true;
    errorMessage.value = '';
    try {
        await cyclesApi.remove(deleteTarget.value.id);
        cycleList.value = cycleList.value.filter(c => c.id !== deleteTarget.value!.id);
        showDeleteModal.value = false;
        deleteTarget.value = null;
    } catch (e) {
        const err = e as PmsApiError;
        if (err.status === 403)      errorMessage.value = 'ไม่มีสิทธิ์ลบข้อมูล (admin เท่านั้น)';
        else if (err.status === 409) errorMessage.value = 'ลบไม่ได้: มีข้อมูลอื่นอ้างอิงถึงรอบนี้';
...
// Reset modal: reload from server (no-op stub like before)
const showResetModal = ref(false);
const resetTarget    = ref<PmsCycle | null>(null);
const confirmReset   = (item: PmsCycle) => { resetTarget.value = item; showResetModal.value = true; };

onMounted(async () => {
    await Promise.all([fetchYears(), fetchList()]);
});
</script>
