            ...
            <div class="flex items-center gap-2">
                <div class="flex h-8 w-8 items-center justify-center rounded-lg" style="background:#fdf4ff;">
                    <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#a855f7" stroke-width="1.8">
                        ...
                        <path d="M12 8v4M7 14h10M5 14v6h14v-6" stroke-linecap="round"/>
                    </svg>
                </div>
                <h1 class="text-lg font-bold text-gray-800">ตำแหน่งงาน</h1>
            </div>
            <NuxtLink
                to="/pms/settings/position/add"
        ...
        <div class="mb-5 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            <div class="flex flex-wrap items-end gap-4">
                ...
                <div class="min-w-[180px]">
                    <label class="mb-1.5 block text-sm font-semibold text-gray-700">รหัสตำแหน่งย่อ</label>
                    <input
                        v-model="searchCode"
                        type="text"
                        class="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition"
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-200 bg-gray-50">
                            ...
                            <th class="px-4 py-3 text-center font-semibold text-gray-700">ชื่อตำแหน่งงาน</th>
                            <th class="w-32 px-4 py-3 text-center font-semibold text-gray-700">รหัสตำแหน่งย่อ</th>
                    ...
                    <tbody>
                        ...
                        <tr v-else-if="positionList.length === 0">
                        ...
                        >
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    <NuxtLink
                                        :to="`/pms/settings/position/add?id=${item.id}`"
            ...
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
                <div class="mb-4 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">ต้องการลบตำแหน่ง "{{ deleteTarget?.name || '-' }}" ใช่หรือไม่?</p>
                    </div>
                </div>
            ...
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
                <div class="mb-4 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">ต้องการรีเซ็ตตำแหน่ง "{{ resetTarget?.name || '-' }}" ใช่หรือไม่?</p>
                    </div>
                </div>
                <div class="flex justify-end gap-2">
...
useHead({ title: 'ตำแหน่งงาน | ระบบประเมินผลการปฏิบัติงาน' });
...
const positionList = ref<PmsPosition[]>([]);
...
// Filter team dropdown by selected department
...
const fetchMasters = async () => {
    try {
        ...
        console.warn('[position] failed to load masters', e);
    }
};
...
const handleClear  = () => {
    searchDept.value = ''; searchTeam.value = ''; searchCode.value = '';
    fetchList();
};

const toggleStatus = async (item: PmsPosition) => {
    togglingId.value = item.id;
    errorMessage.value = '';
    try {
        const res = await positionsApi.update(item.id, { is_active: !item.is_active });
        const idx = positionList.value.findIndex(p => p.id === item.id);
        if (idx >= 0) positionList.value[idx] = res.data;
    } catch (e) {
...
const deleteTarget    = ref<PmsPosition | null>(null);
const confirmDelete   = (item: PmsPosition) => { deleteTarget.value = item; showDeleteModal.value = true; };
const executeDelete   = async () => {
    if (!deleteTarget.value) return;
    deleting.value = true;
    errorMessage.value = '';
    try {
        await positionsApi.remove(deleteTarget.value.id);
        positionList.value = positionList.value.filter(p => p.id !== deleteTarget.value!.id);
        showDeleteModal.value = false;
        deleteTarget.value = null;
    } catch (e) {
        const err = e as PmsApiError;
        if (err.status === 403)      errorMessage.value = 'ไม่มีสิทธิ์ลบข้อมูล (admin เท่านั้น)';
        else if (err.status === 409) errorMessage.value = 'ลบไม่ได้: มีพนักงานอ้างอิงถึงตำแหน่งนี้';
...
const resetTarget    = ref<PmsPosition | null>(null);
const confirmReset   = (item: PmsPosition) => { resetTarget.value = item; showResetModal.value = true; };
