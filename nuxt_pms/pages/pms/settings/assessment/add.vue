                            ...
                            <tbody>
                                <template v-for="(kpi, idx) in kpiRows" :key="idx">
                                    ...
                                    <tr class="border-b border-gray-100">
                                        ...
                                        <td class="px-2 py-2">
                                            <input v-model="kpi.target" type="text" placeholder="เป้าหมาย" class="w-full rounded-lg border border-gray-200 px-2 py-1.5 text-sm text-gray-700 outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-100 transition" />
                                        </td>
                            ...
                            <tbody>
                                <template v-for="(comp, idx) in competencyRows" :key="idx">
                                    ...
                                    <tr class="border-b border-gray-100">
                                        ...
                                        <td class="px-2 py-2">
                                            <input v-model="comp.target" type="text" placeholder="เป้าหมาย" class="w-full rounded-lg border border-gray-200 px-2 py-1.5 text-sm text-gray-700 outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-100 transition" />
                                        </td>
                                        <td class="px-2 py-2 text-center">
...
// ── KPI rows (UI uses string for input compatibility) ─────────────────
interface KpiRow { subject: string; detail: string; weight: string; target: string; options: string[]; }
const makeKpiRow = (): KpiRow => ({ subject: '', detail: '', weight: '', target: '', options: ['', '', '', '', ''] });
...
// ── Competency rows ────────────────────────────────────────────────────
interface CompetencyRow { subject: string; weight: string; target: string; options: string[]; }
const makeCompRow = (): CompetencyRow => ({ subject: '', weight: '', target: '', options: ['', '', '', '', ''] });
