    ...
    <div>
        ...
        <div class="mb-5 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <h1 class="text-lg font-bold text-gray-800">เกณฑ์การประเมินผล</h1>
            </div>
            <NuxtLink
                to="/pms/settings/criteria/add"
        ...
        <div class="mb-5 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            <div class="flex flex-wrap items-end gap-4">
                <div class="min-w-[260px]">
                    <label class="mb-1.5 block text-sm font-semibold text-gray-700">ชื่อเกณฑ์การประเมิน</label>
                    <input
                        v-model="searchName"
                        type="text"
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-200 bg-gray-50">
                        ...
                        <th class="px-5 py-3 text-center font-semibold text-gray-700">ชื่อเกณฑ์การประเมิน</th>
                ...
                <tbody>
                    ...
                    <tr v-else-if="criteriaList.length === 0">
                        <td colspan="4" class="px-5 py-10 text-center text-gray-400">ไม่พบข้อมูล</td>
                    </tr>
                    <tr
                        v-else
                    ...
                    >
                        ...
                        <td class="px-5 py-3 text-center">
                            <div class="flex items-center justify-center gap-1.5">
                                <NuxtLink
                                    :to="`/pms/settings/criteria/add?id=${item.id}`"
            ...
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
                <div class="mb-4 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">ต้องการลบเกณฑ์ "{{ deleteTarget?.name }}" ใช่หรือไม่?</p>
                    </div>
                </div>
            ...
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
                <div class="mb-4 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">ต้องการรีเซ็ตเกณฑ์ "{{ resetTarget?.name }}" ใช่หรือไม่?</p>
                    </div>
                </div>
                <div class="flex justify-end gap-2">
...
useHead({ title: 'เกณฑ์การประเมินผล | ระบบประเมินผลการปฏิบัติงาน' });
definePageMeta({ layout: 'pms-layout' });

const api = usePmsCriteria();

const criteriaList = ref<PmsCriteria[]>([]);
...
const handleClear  = () => { searchName.value = ''; searchStatus.value = ''; fetchList(); };

const toggleStatus = async (item: PmsCriteria) => {
    togglingId.value = item.id;
    errorMessage.value = '';
    try {
        // PATCH only is_active — `grades` not sent so RPC won't replace them
        const res = await api.update(item.id, { is_active: !item.is_active });
        const idx = criteriaList.value.findIndex(c => c.id === item.id);
        if (idx >= 0) criteriaList.value[idx] = res.data;
    } catch (e) {
...
const deleteTarget    = ref<PmsCriteria | null>(null);
const confirmDelete   = (item: PmsCriteria) => { deleteTarget.value = item; showDeleteModal.value = true; };
const executeDelete   = async () => {
    ...
    try {
        ...
        criteriaList.value = criteriaList.value.filter(c => c.id !== deleteTarget.value!.id);
        showDeleteModal.value = false;
        deleteTarget.value = null;
    } catch (e) {
        const err = e as PmsApiError;
        if (err.status === 403)      errorMessage.value = 'ไม่มีสิทธิ์ลบข้อมูล (admin เท่านั้น)';
        else if (err.status === 409) errorMessage.value = 'ลบไม่ได้: มีแบบประเมินอ้างอิงถึงเกณฑ์นี้';
...
const resetTarget    = ref<PmsCriteria | null>(null);
const confirmReset   = (item: PmsCriteria) => { resetTarget.value = item; showResetModal.value = true; };
