                        ...
                        <table class="w-full text-sm">
                            ...
                            <tbody>
                                <tr
                                    ...
                                    :class="conflictRowIndices.has(index) ? 'bg-red-50' : ''"
                                >
                                    <!-- ระดับแต้มคะแนน -->
                                    <td class="px-3 py-2">
                                        <input
...
// Indices of rows currently flagged as overlapping (visual highlight).
const conflictRowIndices = ref<Set<number>>(new Set());
...
// Detect the first overlapping pair of grade rows. Mirrors the inclusive-on-both-ends
// semantics of the DB EXCLUDE constraint and the edge function's findOverlap().
// Returns the two original row indices (in display order) so callers can highlight them.
const detectGradeOverlap = (): { a: number; b: number } | null => {
    const indexed = gradeRows.value
        .map((r, i) => ({
            i,
...
const validate = (): boolean => {
    ...
    conflictRowIndices.value = new Set();
    let valid = true;
    ...
    // Inter-row overlap check (runs only after per-row checks pass to avoid
    // confusing the user with multiple errors at once).
    if (valid) {
        const overlap = detectGradeOverlap();
        if (overlap) {
            const a = gradeRows.value[overlap.a];
            const b = gradeRows.value[overlap.b];
            errors.grades = `ช่วงคะแนนทับซ้อนกัน: เกรด '${a.grade}' (${a.minScore}-${a.maxScore}) ทับซ้อนกับ '${b.grade}' (${b.minScore}-${b.maxScore})`;
            conflictRowIndices.value = new Set([overlap.a, overlap.b]);
            valid = false;
        }
    }
...
interface OverlapDetail {
    code?: 'overlap';
    conflicting?: Array<{ grade: string; min_score: number; max_score: number }>;
}

const handleApiError = (e: unknown) => {
    const err = e as PmsApiError;
    const details = err.details as OverlapDetail | null;

    // Server-side overlap (race-protection from DB or pre-RPC validator).
    // Map the conflicting grade rows back to their indices so we can highlight them.
    if (err.status === 422 && details?.code === 'overlap') {
        const indices = new Set<number>();
        for (const c of details.conflicting ?? []) {
            const idx = gradeRows.value.findIndex(
                r => r.grade.trim() === c.grade
                    && parseFloat(r.minScore) === c.min_score
                    && parseFloat(r.maxScore) === c.max_score,
            );
            if (idx >= 0) indices.add(idx);
        }
        conflictRowIndices.value = indices;
        errors.grades = err.message || 'ช่วงคะแนนทับซ้อนกัน';
        return;
    }
