    ...
    <div>
        ...
        <div class="mb-5 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <h1 class="text-lg font-bold text-gray-800">ระดับตำแหน่ง</h1>
            </div>
            <NuxtLink
                to="/pms/settings/level/add"
        ...
        <div class="mb-5 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            <div class="flex flex-wrap items-end gap-4">
                <div class="min-w-[240px]">
                    <label class="mb-1.5 block text-sm font-semibold text-gray-700">ชื่อระดับตำแหน่ง</label>
                    <input
                        v-model="searchName"
                        type="text"
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-200 bg-gray-50">
                        ...
                        <th class="px-5 py-3 text-center font-semibold text-gray-700">ชื่อระดับตำแหน่ง</th>
                        <th class="w-36 px-5 py-3 text-center font-semibold text-gray-700">การจัดเรียง</th>
                ...
                <tbody>
                    ...
                    <tr v-else-if="levelList.length === 0">
                    ...
                    >
                        ...
                        <td class="px-5 py-3 text-center text-gray-600">{{ item.sort_order }}</td>
                        ...
                        <td class="px-5 py-3 text-center">
                            <div class="flex items-center justify-center gap-1.5">
                                <NuxtLink
                                    :to="`/pms/settings/level/add?id=${item.id}`"
            ...
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
                <div class="mb-4 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">ต้องการลบระดับตำแหน่ง "{{ deleteTarget?.name }}" ใช่หรือไม่?</p>
                    </div>
                </div>
            ...
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
                <div class="mb-4 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">ต้องการรีเซ็ตระดับตำแหน่ง "{{ resetTarget?.name }}" ใช่หรือไม่?</p>
                    </div>
                </div>
                <div class="flex justify-end gap-2">
...
useHead({ title: 'ระดับตำแหน่ง | ระบบประเมินผลการปฏิบัติงาน' });
definePageMeta({ layout: 'pms-layout' });

const api = usePmsLevels();

const levelList    = ref<PmsLevel[]>([]);
...
const handleClear  = () => { searchName.value = ''; fetchList(); };

const toggleStatus = async (item: PmsLevel) => {
    togglingId.value = item.id;
    errorMessage.value = '';
    try {
        const res = await api.update(item.id, { is_active: !item.is_active });
        const idx = levelList.value.findIndex(l => l.id === item.id);
        if (idx >= 0) levelList.value[idx] = res.data;
    } catch (e) {
...
const deleteTarget    = ref<PmsLevel | null>(null);
const confirmDelete   = (item: PmsLevel) => { deleteTarget.value = item; showDeleteModal.value = true; };
const executeDelete   = async () => {
    ...
    try {
        ...
        levelList.value = levelList.value.filter(l => l.id !== deleteTarget.value!.id);
        showDeleteModal.value = false;
        deleteTarget.value = null;
    } catch (e) {
        const err = e as PmsApiError;
        if (err.status === 403)      errorMessage.value = 'ไม่มีสิทธิ์ลบข้อมูล (admin เท่านั้น)';
        else if (err.status === 409) errorMessage.value = 'ลบไม่ได้: มีข้อมูลอื่นอ้างอิงถึงระดับนี้';
...
const resetTarget    = ref<PmsLevel | null>(null);
const confirmReset   = (item: PmsLevel) => { resetTarget.value = item; showResetModal.value = true; };
